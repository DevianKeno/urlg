...
Purpose:
    ...
    Generated datasets are written to structured CSV files.
...
Data Structures:
    EntryCount: total number of dataset entries to generate
    RoomsPerPlayerPreference: number of rooms paired with each player preference
    FluctuationChance: probability of deviating from the classification threshold (used for testing only)
    ARDataEntry: represents a single data entry consisting of player and room seeds, feature values, and ground truth labels
    ResultsJsonData: container for parsed JSON data entries from external sources
*/
